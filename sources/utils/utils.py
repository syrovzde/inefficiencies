...
import sources.utils.helper_functions as hlp
import os
...
def load_asian_odds(engine,MatchID,market,timestamp,from_csv=False):
    if market == 'ou':
        if from_csv:
            odds = pd.read_csv('csv_files' + os.sep +'ou_sample.csv')
    ...
    if market == 'ah':
        if from_csv:
            odds = pd.read_csv("csv_files" + os.sep + "ah_sample.csv")
            odds= odds[odds['Timestamp'] == timestamp]
    ...
    if market == '1x2':
        if from_csv:
            odds = pd.read_csv('csv_files' + os.sep + '1x2_sample.csv')
